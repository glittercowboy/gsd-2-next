...
6. Write `{{milestoneSummaryPath}}` using the milestone-summary template. Fill all frontmatter fields and narrative sections. The `requirement_outcomes` field must list every requirement that changed status with `from_status`, `to_status`, and `proof`. If a **Pre-Computed Aggregation** section is inlined above, use its `key_files`, `key_decisions`, `patterns_established`, `duration`, and `verification_result` values — the aggregation is already deduplicated and summed from slice summaries.
